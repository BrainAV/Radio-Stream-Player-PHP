...
if (isset($_SESSION['user_id'])) {
    ...
    $accountBtnHtml = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">';
    $accountBtnHtml .= '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/>';
    $accountBtnHtml .= '</svg>';

    $accountBtn = '<button id="header-account-btn" class="theme-btn desktop-only" style="font-size: 14px; display: flex; align-items: center;" aria-label="My Account" title="My Account">' . $accountBtnHtml . '</button>';
    $accountBtnMob = '<button id="mobile-account-btn" class="mobile-nav-item" aria-label="My Account">' . $accountBtnHtml . '<span>My Account</span></button>';
    
    $logoutBtnHtml = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">';
    $logoutBtnHtml .= '<path d="M17 7l-1.41 1.41L18.17 11H8v2h10.17l-2.58 2.58L17 17l5-5-5-5zM4 5h8V3H4c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h8v-2H4V5z"/>';
    $logoutBtnHtml .= '</svg>';

    $logoutBtn = '<button id="header-logout-btn" class="theme-btn desktop-only" style="font-size: 14px; display: flex; align-items: center;" aria-label="Logout" title="Logout">' . $logoutBtnHtml . '</button>';
    $logoutBtnMob = '<button id="mobile-logout-btn" class="mobile-nav-item" aria-label="Logout">' . $logoutBtnHtml . '<span>Logout</span></button>';
    
    $html = preg_replace('/<\/div>\s*<\/header>/i', "\n" . $adminBtn . "\n" . $accountBtn . "\n" . $logoutBtn . "\n        </div>\n    </header>", $html);
    $html = str_replace('<!-- Buttons injected by index.php -->', $adminBtnMob . $accountBtnMob . $logoutBtnMob, $html);
    ...
    $userRole = null;
    $loginBtnHtml = '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">';
    $loginBtnHtml .= '<path d="M11 7L9.6 8.4l2.6 2.6H2v2h10.2l-2.6 2.6L11 17l5-5-5-5zm9 12h-8v2h8c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-8v2h8v14z"/>';
    $loginBtnHtml .= '</svg>';

    $loginBtn = '<button id="header-login-btn" class="theme-btn desktop-only" style="font-size: 14px; display: flex; align-items: center;" aria-label="Login" title="Login">' . $loginBtnHtml . '</button>';
    $loginBtnMob = '<button id="mobile-login-btn" class="mobile-nav-item" aria-label="Login">' . $loginBtnHtml . '<span>Login</span></button>';
    
    $html = preg_replace('/<\/div>\s*<\/header>/i', "\n" . $loginBtn . "\n        </div>\n    </header>", $html);
    $html = str_replace('<!-- Buttons injected by index.php -->', $loginBtnMob, $html);
    $html = preg_replace('/<body[^>]*>/i', "$0\n" . $guestJs, $html);
}
